...
public sealed class UserFacadeTests : CRUDFacadeTestsBase
{
    ...
    public async Task NewUser_InsertOrUpdate_UserAdded()
    {
        ...
        var user = new UserDetailModel
        (
            UserSeeds.UserEntity2.Name,
            UserSeeds.UserEntity2.Surname,
            UserSeeds.UserEntity2.PhotoUrl
        )
        {
    ...
    public async Task SeededUserEntity_InsertOrUpdate_UserUpdated()
    {
        ...
        //Assert
        await using var dbxAssert = await DbContextFactory.CreateDbContextAsync();
        var userFromDb = await dbxAssert.Users.Include(x => x.OwnedCars).Include(x => x.PassengerRides)
